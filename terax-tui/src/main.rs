...
#[derive(Clone, Copy, Debug, Eq, PartialEq)] enum View { Workspace, Ai, Git, Terminal, Editor, Diff }
...
fn ui(f:&mut Frame, app:&mut App){ app.poll_ai(); app.poll_pty(); let root=Layout::default().direction(Direction::Vertical).constraints([Constraint::Min(8),Constraint::Length(3),Constraint::Length(7)]).split(f.area()); let main=Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(34),Constraint::Percentage(66)]).split(root[0]); render_files(f,app,main[0]); if app.view==View::Ai{render_chat(f,app,main[1])}else if app.view==View::Git{render_git(f,app,main[1])}else if app.view==View::Terminal{render_terminal(f,app,main[1])}else if app.view==View::Editor{render_editor(f,app,main[1])}else if app.view==View::Diff{render_diff(f,app,main[1])}else{render_preview(f,app,main[1])} render_command(f,app,root[1]); render_log(f,app,root[2]); }
