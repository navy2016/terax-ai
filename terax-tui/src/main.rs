...
#[derive(Debug)] enum AiEvent { Done(String), EditProposal(String), Error(String) }
...
impl App {
    ...
    fn poll_ai(&mut self){ if let Some(rx)=self.ai_rx.take(){ match rx.try_recv(){ Ok(AiEvent::Done(ans))=>{self.ai_busy=false; self.status="AI done".into(); self.chat.push(ai::assistant(ans.clone())); self.logs.push("AI done".into());}, Ok(AiEvent::EditProposal(new_line))=>{self.ai_busy=false; self.status="AI edit proposed".into(); self.create_pending_line_edit(new_line);}, Ok(AiEvent::Error(e))=>{self.ai_busy=false; self.status="AI error".into(); self.logs.push(format!("AI error: {e}")); self.chat.push(ai::assistant(format!("Error: {e}")));}, Err(mpsc::TryRecvError::Empty)=>{ self.ai_rx=Some(rx); }, Err(mpsc::TryRecvError::Disconnected)=>{self.ai_busy=false; self.status="AI disconnected".into();} } } }
...
fn render_log(f:&mut Frame, app:&App, area:Rect){ let h=area.height.saturating_sub(2) as usize; let start=app.logs.len().saturating_sub(h); f.render_widget(Paragraph::new(app.logs[start..].join("\n")).block(Block::default().title(if app.pending_edit.is_some(){" Log [PENDING EDIT: a/n] "}else if app.pending_tool.is_some(){" Log [PENDING TOOL: y/n] "}else{" Log "}).borders(Borders::ALL).border_style(focus_style(app.focus==Focus::Log))).wrap(Wrap{trim:false}),area)}
