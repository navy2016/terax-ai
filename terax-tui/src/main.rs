...
use terax_core::{ai::{self, ChatMessage}, config::Config, git::GitEntry};
use walkdir::WalkDir;

#[derive(Clone, Copy, Debug, Eq, PartialEq)] enum Focus { Files, Git, Command, Chat, Log }
#[derive(Clone, Copy, Debug, Eq, PartialEq)] enum View { Workspace, Ai, Git }
...
struct App {
    ...
    chat: Vec<ChatMessage>, ai_busy: bool, ai_rx: Option<Receiver<AiEvent>>, status: String, git_entries: Vec<GitEntry>, git_selected: usize, git_diff: String,
}
impl App {
    fn new() -> Result<Self> { let cwd=env::current_dir()?; let mut a=Self{cwd,entries:vec![],selected:0,preview:String::new(),command:String::new(),logs:vec!["Terax TUI v0.3".into(),"Ctrl+A AI | Ctrl+G Git | :ai | /explain | /review-diff | /commit-message | q".into()],focus:Focus::Files,view:View::Workspace,should_quit:false,chat:vec![ai::system("You are Terax TUI assistant. Be concise, practical, and code-aware.")],ai_busy:false,ai_rx:None,status:"AI idle".into(),git_entries:vec![],git_selected:0,git_diff:String::new()}; a.refresh()?; a.refresh_git(); Ok(a) }
