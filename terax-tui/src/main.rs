...
#[derive(Clone, Copy, Debug, Eq, PartialEq)] enum Focus { Files, Git, Terminal, Editor, Diff, Command, Chat, Log }
#[derive(Clone, Copy, Debug, Eq, PartialEq)] enum View { Workspace, Ai, Git, Terminal, Editor, Diff, Help }
#[derive(Debug)] enum AiEvent { Done(String), Agent(String), EditProposal(String), BufferProposal(Option<PathBuf>, String), Error(String) }
...
impl App {
    ...
    fn poll_ai(&mut self){ if let Some(rx)=self.ai_rx.take(){ match rx.try_recv(){ Ok(AiEvent::Done(ans))=>{self.ai_busy=false; self.status="AI done".into(); self.chat.push(ai::assistant(ans.clone())); self.logs.push("AI done".into());}, Ok(AiEvent::Agent(ans))=>{self.ai_busy=false; self.status="agent response".into(); self.handle_agent_response(ans);}, Ok(AiEvent::EditProposal(new_line))=>{self.ai_busy=false; self.status="AI edit proposed".into(); self.create_pending_line_edit(new_line);}, Ok(AiEvent::BufferProposal(path,new_text))=>{self.ai_busy=false; self.status="AI buffer edit proposed".into(); self.create_pending_buffer_edit_for(path,new_text);}, Ok(AiEvent::Error(e))=>{self.ai_busy=false; self.status="AI error".into(); self.logs.push(format!("AI error: {e}")); self.chat.push(ai::assistant(format!("Error: {e}")));}, Err(mpsc::TryRecvError::Empty)=>{ self.ai_rx=Some(rx); }, Err(mpsc::TryRecvError::Disconnected)=>{self.ai_busy=false; self.status="AI disconnected".into();} } } }
    ...
    fn open_path(&mut self, path_text:&str){ let p=terax_core::fs::canonical_or_original(self.cwd.join(path_text)); if p.is_dir(){ self.cwd=p; self.selected=0; let _=self.refresh(); self.refresh_git(); self.logs.push(format!("opened {}", self.cwd.display())); } else { self.logs.push("open: not a directory".into()); } }
    fn clone_repo_cmd(&mut self, url:&str){ match terax_core::git::clone_repo(&self.cwd, url){Ok(o)=>{self.logs.push(o); let name=url.trim_end_matches('/').split('/').last().unwrap_or("").trim_end_matches(".git"); let p=self.cwd.join(name); if p.is_dir(){self.cwd=terax_core::fs::canonical_or_original(p); let _=self.refresh(); self.refresh_git();}},Err(e)=>self.logs.push(format!("clone error: {e:#}"))} }
    fn set_remote_cmd(&mut self, url:&str){ match terax_core::git::set_remote_origin(&self.cwd,url){Ok(o)=>{if !o.trim().is_empty(){self.logs.push(o)} self.logs.push(format!("origin -> {}", url));},Err(e)=>self.logs.push(format!("set remote error: {e:#}"))} }
    fn commit_cmd(&mut self, msg:&str){ match terax_core::git::commit(&self.cwd,msg){Ok(o)=>{self.logs.push(o); self.refresh_git();},Err(e)=>self.logs.push(format!("commit error: {e:#}"))} }
    fn agent_commit_message(&mut self){ self.commit_message(); }

    fn project_root(&self)->PathBuf{ terax_core::fs::find_project_root(&self.cwd) }
    fn show_project_root(&mut self){ let root=self.project_root(); self.logs.push(format!("project root: {}", root.display())); }
    fn build_index(&mut self){ let root=self.project_root(); let idx=terax_core::fs::workspace_index(&root, 400); let path=root.join(".terax-index.txt"); match std::fs::write(&path,&idx){Ok(_)=>{self.logs.push(format!("index written: {}", path.display())); self.editor_path=Some(path); self.editor_lines=idx.lines().map(|s|s.to_string()).collect(); self.view=View::Editor; self.focus=Focus::Editor; self.editor_dirty=false;},Err(e)=>self.logs.push(format!("index write error: {e:#}"))} }
    fn open_memory(&mut self){ let root=self.project_root(); let path=root.join("TERAX.md"); if !path.exists(){ let _=std::fs::write(&path,"# TERAX Project Memory\n\n- Project notes go here.\n"); } self.open_editor(path); }
    fn init_repo(&mut self){ let out=Command::new("git").arg("init").current_dir(&self.cwd).output(); match out{Ok(o)=>{self.logs.push(String::from_utf8_lossy(&o.stdout).to_string()); if !o.stderr.is_empty(){self.logs.push(String::from_utf8_lossy(&o.stderr).to_string());} self.refresh_git();},Err(e)=>self.logs.push(format!("git init error: {e:#}"))} }
...
{"tool":"git_status"}
{"tool":"git_diff"}
{"tool":"terminal_send","command":"shell command"}
Choose exactly one."#;
        let root=terax_core::fs::find_project_root(&self.cwd);
        let memory=std::fs::read_to_string(root.join("TERAX.md")).unwrap_or_default();
        let context=format!("cwd: {}\nproject_root: {}\nTERAX.md memory:\n{}\n\nTask: {}", self.cwd.display(), root.display(), memory, task);
        let msgs=vec![ai::system(system), ai::user(context)];
    ...
    fn handle_agent_response(&mut self, text: String){
        let trimmed=extract_json_object(&text).unwrap_or_else(|| text.trim().trim_matches('`').trim().to_string());
        let parsed:Result<serde_json::Value,_>=serde_json::from_str(&trimmed);
        let Ok(v)=parsed else { self.logs.push(format!("agent raw: {}", text)); self.chat.push(ai::assistant(text)); return; };
        if let Some(ans)=v.get("answer").and_then(|x|x.as_str()){
            self.logs.push(format!("agent: {}", ans));
            self.chat.push(ai::assistant(ans.to_string()));
            return;
        }
        ...
        match tool{
            "terminal_send"=>{
                ...
                if cmd.is_empty(){self.logs.push("agent terminal_send missing command".into())} else {self.set_pending(ToolRequest::terminal_send(cmd));}
            }
            "read_file"=>{
                let path=v.get("path").and_then(|x|x.as_str()).unwrap_or("");
                if path.is_empty(){self.logs.push("agent read_file missing path".into())} else {self.set_pending(ToolRequest{ id:format!("read-file-{}", self.logs.len()), kind:ToolKind::ReadFile, summary:format!("Read file: {}", path), payload:path.into(), risk:ToolRisk::Low });}
            }
            "git_status"=>self.set_pending(ToolRequest{ id:format!("git-status-{}", self.logs.len()), kind:ToolKind::GitStatus, summary:"Read git status".into(), payload:String::new(), risk:ToolRisk::Low }),
            "git_diff"=>self.set_pending(ToolRequest{ id:format!("git-diff-{}", self.logs.len()), kind:ToolKind::GitDiff, summary:"Read git diff".into(), payload:String::new(), risk:ToolRisk::Low }),
            _=>self.logs.push(format!("agent unknown tool response: {}", trimmed)),
        }
    }

    fn ask_ai_write_file(&mut self, path_text: String, instruction: String){
        if self.ai_busy { self.logs.push("AI busy".into()); return; }
        let path=self.cwd.join(path_text.trim());
        let existing=std::fs::read_to_string(&path).unwrap_or_default();
        let prompt=format!("You are creating or rewriting a file. Return ONLY the full file content, no markdown fences, no explanation.\nTarget file: {}\nInstruction: {}\n\nExisting content, possibly empty:\n{}", path.display(), instruction, existing);
        let msgs=vec![ai::system("Return only full file content. No markdown fences. No explanation."), ai::user(prompt)];
        let (tx,rx)=mpsc::channel(); self.ai_rx=Some(rx); self.ai_busy=true; self.status="AI writing file...".into();
        thread::spawn(move||{ let _=tx.send(match ai::chat(msgs){Ok(a)=>AiEvent::BufferProposal(Some(path), strip_code_fence(a)),Err(e)=>AiEvent::Error(format!("{e:#}"))}); });
    }
    ...
    fn ask_ai_edit_buffer(&mut self, instruction: String){
        ...
        thread::spawn(move||{ let _=tx.send(match ai::chat(msgs){Ok(a)=>AiEvent::BufferProposal(None, strip_code_fence(a)),Err(e)=>AiEvent::Error(format!("{e:#}"))}); });
    }
    fn create_pending_buffer_edit(&mut self, new_text: String){ self.create_pending_buffer_edit_for(None, new_text); }
    fn create_pending_buffer_edit_for(&mut self, path: Option<PathBuf>, new_text: String){
        let target_path=path.or_else(|| self.editor_path.clone());
        let old_lines=if let Some(p)=&target_path{ std::fs::read_to_string(p).map(|t|t.lines().map(|s|s.to_string()).collect()).unwrap_or_else(|_| self.editor_lines.clone()) }else{ self.editor_lines.clone() };
        let new_lines:Vec<String>=new_text.lines().map(|s|s.to_string()).collect();
        let old_count=old_lines.len();
        let new_count=new_lines.len();
        self.pending_buffer_edit=Some(PendingBufferEdit{path:target_path.clone(), old_lines, new_lines});
        self.logs.push(format!("PENDING FILE EDIT: {} lines -> {} lines", old_count, new_count));
        if let Some(p)=target_path{ self.logs.push(format!("target: {}", p.display())); }
        self.logs.push("Review with Ctrl+D, apply? a / n".into());
    }
    fn apply_pending_buffer_edit(&mut self){
        let Some(edit)=self.pending_buffer_edit.take() else { return; };
        self.editor_path=edit.path;
    ...
    fn approve_pending(&mut self){
        ...
        match req.kind {
            ToolKind::TerminalSend => {
                ...
                self.logs.push(format!("approved tool: {}", req.summary)); self.logs.push("terminal_send executed; collecting output for agent...".into()); self.terminal_agent_wait=Some(12);
            }
            ToolKind::ReadFile => {
    ...
    fn poll_pty(&mut self){
        ...
        if let Some(wait)=self.terminal_agent_wait {
            if wait == 0 {
                self.terminal_agent_wait=None;
                let tail=self.terminal_tail(120);
                if !tail.trim().is_empty(){ self.continue_agent_with_tool_result("terminal_send", tail); }
            } else {
                self.terminal_agent_wait=Some(wait.saturating_sub(1));
            }
        }
    }
    ...
    fn run_command(&mut self){ let cmd=self.command.trim().to_string(); if cmd.is_empty(){return} self.logs.push(format!("> {cmd}")); if cmd=="quit"||cmd=="exit"{self.should_quit=true;return} if cmd=="pwd"{self.logs.push(self.cwd.display().to_string()); self.command.clear(); return} if cmd=="/explain"{self.explain_current(); self.command.clear(); return} if cmd=="/review-diff"{self.review_diff(); self.command.clear(); return} if cmd=="/commit-message"{self.commit_message(); self.command.clear(); return} if cmd=="/terminal-tail"{self.ask_terminal_tail(); self.command.clear(); return} if cmd=="/git-status"{self.ask_git_status(); self.command.clear(); return} if cmd=="/project-root"||cmd=="project-root"{self.show_project_root(); self.command.clear(); return} if cmd=="/index"||cmd=="index"{self.build_index(); self.command.clear(); return} if cmd=="/memory"||cmd=="memory"{self.open_memory(); self.command.clear(); return} if cmd=="/init-repo"||cmd=="init-repo"{self.init_repo(); self.command.clear(); return} if cmd=="/agent-commit-message"||cmd=="agent-commit-message"{self.agent_commit_message(); self.command.clear(); return} if let Some(url)=cmd.strip_prefix("clone ").or_else(||cmd.strip_prefix("/clone ")){self.clone_repo_cmd(url.trim()); self.command.clear(); return} if let Some(path)=cmd.strip_prefix("open ").or_else(||cmd.strip_prefix("/open ")){self.open_path(path.trim()); self.command.clear(); return} if let Some(url)=cmd.strip_prefix("set-remote ").or_else(||cmd.strip_prefix("/set-remote ")){self.set_remote_cmd(url.trim()); self.command.clear(); return} if let Some(msg)=cmd.strip_prefix("commit ").or_else(||cmd.strip_prefix("/commit ")){self.commit_cmd(msg.trim()); self.command.clear(); return} if cmd=="diff-preview"||cmd=="/diff-preview"{self.view=View::Diff; self.focus=Focus::Diff; self.command.clear(); return} if let Some(task)=cmd.strip_prefix("agent "){self.ask_agent(task.trim().to_string()); self.command.clear(); return} if let Some(rest)=cmd.strip_prefix("ai-write-file "){let mut parts=rest.trim().splitn(2, char::is_whitespace); let path=parts.next().unwrap_or("").to_string(); let inst=parts.next().unwrap_or("").trim().to_string(); if path.is_empty()||inst.is_empty(){self.logs.push("usage: ai-write-file <path> <instruction>".into())}else{self.ask_ai_write_file(path,inst)} self.command.clear(); return} if let Some(inst)=cmd.strip_prefix("ai-edit-buffer "){self.ask_ai_edit_buffer(inst.trim().to_string()); self.command.clear(); return} if let Some(inst)=cmd.strip_prefix("ai-edit-line "){self.ask_ai_edit_line(inst.trim().to_string()); self.command.clear(); return} if let Some(p)=cmd.strip_prefix("ai "){self.ask_ai(p.trim().to_string()); self.command.clear(); return} if let Some(c)=cmd.strip_prefix("run "){self.set_pending(ToolRequest::terminal_send(c.trim())); self.command.clear(); return} if let Some(rest)=cmd.strip_prefix("cd "){let t=self.cwd.join(rest.trim()); if t.is_dir(){self.cwd=terax_core::fs::canonical_or_original(t); self.selected=0; let _=self.refresh();} self.command.clear(); return} let output=Command::new("sh").arg("-lc").arg(&cmd).current_dir(&self.cwd).output(); match output{Ok(o)=>{for l in String::from_utf8_lossy(&o.stdout).lines().take(120){self.logs.push(l.into())} for l in String::from_utf8_lossy(&o.stderr).lines().take(120){self.logs.push(format!("ERR: {l}"))} self.logs.push(format!("exit: {}",o.status));},Err(e)=>self.logs.push(format!("command failed: {e}"))} self.command.clear(); }
    fn on_key(&mut self,key:KeyEvent)->Result<()> { if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('c'){self.should_quit=true; return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('a'){self.view=if self.view==View::Ai{View::Workspace}else{View::Ai}; self.focus=Focus::Command; return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('g'){self.view=if self.view==View::Git{View::Workspace}else{View::Git}; self.focus=Focus::Git; self.refresh_git(); return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('t'){self.view=if self.view==View::Terminal{View::Workspace}else{View::Terminal}; self.focus=Focus::Terminal; self.ensure_pty(); return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('e'){self.view=if self.view==View::Editor{View::Workspace}else{View::Editor}; self.focus=Focus::Editor; return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('d'){self.view=if self.view==View::Diff{View::Workspace}else{View::Diff}; self.focus=Focus::Diff; return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('h'){self.view=if self.view==View::Help{View::Workspace}else{View::Help}; return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('l') && self.focus==Focus::Terminal {self.terminal_lines.clear(); return Ok(())} if key.modifiers.contains(KeyModifiers::CONTROL)&&key.code==KeyCode::Char('s') && self.focus==Focus::Editor {self.save_editor(); return Ok(())} if self.pending_buffer_edit.is_some(){ match key.code { KeyCode::Char('a')=>{self.apply_pending_buffer_edit(); return Ok(())}, KeyCode::Char('n')|KeyCode::Esc=>{self.deny_pending_buffer_edit(); return Ok(())}, _=>{} } } if self.pending_edit.is_some(){ match key.code { KeyCode::Char('a')=>{self.apply_pending_edit(); return Ok(())}, KeyCode::Char('n')|KeyCode::Esc=>{self.deny_pending_edit(); return Ok(())}, _=>{} } } if self.pending_tool.is_some(){ match key.code { KeyCode::Char('y')=>{self.approve_pending(); return Ok(())}, KeyCode::Char('n')|KeyCode::Esc=>{self.deny_pending(); return Ok(())}, _=>{} } } match self.focus{ Focus::Files=>match key.code{KeyCode::Char('q')=>self.should_quit=true,KeyCode::Tab=>self.focus=Focus::Command,KeyCode::Char(':')=>self.focus=Focus::Command,KeyCode::Char('r')=>self.refresh()?,KeyCode::Up=>{self.selected=self.selected.saturating_sub(1);self.update_preview()},KeyCode::Down=>{if self.selected+1<self.entries.len(){self.selected+=1} self.update_preview()},KeyCode::Enter=>{ if let Some(p)=self.selected_path().map(Path::to_path_buf){ if p.is_file(){self.open_editor(p)} else {self.open_selected()?} } },_=>{}}, Focus::Git=>match key.code{KeyCode::Char('q')=>self.should_quit=true,KeyCode::Tab|KeyCode::Esc=>self.focus=Focus::Command,KeyCode::Char('r')=>self.refresh_git(),KeyCode::Char('s')=>self.toggle_stage(),KeyCode::Up=>{self.git_selected=self.git_selected.saturating_sub(1);self.update_git_diff()},KeyCode::Down=>{if self.git_selected+1<self.git_entries.len(){self.git_selected+=1} self.update_git_diff()},_=>{}}, Focus::Terminal=>match key.code{KeyCode::Esc=>self.focus=Focus::Files,KeyCode::Tab=>self.focus=Focus::Command,KeyCode::Enter=>self.terminal_send(),KeyCode::Backspace=>{self.terminal_input.pop();},KeyCode::Char(c)=>self.terminal_input.push(c),_=>{}}, Focus::Editor=>match key.code{KeyCode::Esc=>{self.editor_insert=false; self.focus=Focus::Files},KeyCode::Tab=>self.focus=Focus::Command,KeyCode::Up=>self.editor_move_up(),KeyCode::Down=>self.editor_move_down(),KeyCode::Left=>self.editor_move_left(),KeyCode::Right=>self.editor_move_right(),KeyCode::Enter=>{if self.editor_insert{self.editor_newline()}},KeyCode::Backspace=>{if self.editor_insert{self.editor_backspace()}},KeyCode::Char('i') if !self.editor_insert=>self.editor_insert=true,KeyCode::Char(c)=>{if self.editor_insert{self.editor_insert_char(c)}},_=>{}}, Focus::Diff=>match key.code{KeyCode::Esc|KeyCode::Tab=>self.focus=Focus::Command,KeyCode::Char('a')=>self.apply_pending_buffer_edit(),KeyCode::Char('n')=>self.deny_pending_buffer_edit(),_=>{}}, Focus::Command=>match key.code{KeyCode::Esc=>self.focus=Focus::Files,KeyCode::Tab=>self.focus=if self.view==View::Ai{Focus::Chat}else{Focus::Log},KeyCode::Enter=>self.run_command(),KeyCode::Backspace=>{self.command.pop();},KeyCode::Char(c)=>self.command.push(c),_=>{}}, Focus::Chat|Focus::Log=>match key.code{KeyCode::Tab|KeyCode::Esc=>self.focus=Focus::Files,KeyCode::Char('q')=>self.should_quit=true,_=>{}} } Ok(()) }
}
...
fn extract_json_object(s:&str)->Option<String>{
    ...
    if let Some(start)=t.find('{'){
        let mut depth=0i32;
        for (idx,ch) in t[start..].char_indices(){
            if ch=='{' { depth+=1; }
            if ch=='}' { depth-=1; if depth==0 { return Some(t[start..start+idx+1].to_string()); } }
        }
    }
    None
}
...
fn doctor_text()->String{
    ...
    out.push_str("Terax TUI doctor\n");
    out.push_str(&format!("version: {}\n", env!("CARGO_PKG_VERSION")));
    let cwd=std::env::current_dir().unwrap_or_default();
    out.push_str(&format!("cwd: {}\n", cwd.display()));
    let root=terax_core::fs::find_project_root(&cwd);
    out.push_str(&format!("project_root: {}\n", root.display()));
    out.push_str(&format!("TERAX.md: {}\n", if root.join("TERAX.md").exists(){"present"}else{"missing"}));
    match Config::load_default(){
        Ok(cfg)=>{
            out.push_str("config: ok\n");
            match cfg.resolve_provider(){ Ok(p)=>out.push_str(&format!("provider: {} model={} key=***\n", p.base_url, p.model)), Err(e)=>out.push_str(&format!("provider: ERR {e:#}\n")) }
        }
        Err(e)=>out.push_str(&format!("config: ERR {e:#}\n")),
    }
    out.push_str(&format!("git: {}\n", terax_core::git::status(&std::env::current_dir().unwrap_or_default()).map(|_|"ok".into()).unwrap_or_else(|e|format!("ERR {e:#}"))));
    out.push_str(&format!("branch: {}\n", terax_core::git::branch(&cwd).unwrap_or_else(|_|"-".into())));
    out.push_str(&format!("remote: {}\n", terax_core::git::remote(&cwd).unwrap_or_else(|_|"-".into()).lines().next().unwrap_or("-") ));
    out
}
...
fn ui(f:&mut Frame, app:&mut App){ app.poll_ai(); app.poll_pty(); let root=Layout::default().direction(Direction::Vertical).constraints([Constraint::Min(8),Constraint::Length(3),Constraint::Length(7)]).split(f.area()); let main=Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(34),Constraint::Percentage(66)]).split(root[0]); render_files(f,app,main[0]); if app.view==View::Ai{render_chat(f,app,main[1])}else if app.view==View::Git{render_git(f,app,main[1])}else if app.view==View::Terminal{render_terminal(f,app,main[1])}else if app.view==View::Editor{render_editor(f,app,main[1])}else if app.view==View::Diff{render_diff(f,app,main[1])}else if app.view==View::Help{render_help(f,app,main[1])}else{render_preview(f,app,main[1])} render_command(f,app,root[1]); render_log(f,app,root[2]); }
...
fn render_diff(f:&mut Frame, app:&mut App, area:Rect){
    let text=if let Some(edit)=&app.pending_buffer_edit{
        make_line_diff(&edit.old_lines,&edit.new_lines, area.height.saturating_sub(2) as usize * 2)
    }else if let Some(edit)=&app.pending_edit{
        format!("Pending line edit at line {}\n- {}\n+ {}", edit.row+1, edit.old, edit.new)
    }else{
        "No pending edit. Use ai-edit-line or ai-edit-buffer first.".into()
    };
    f.render_widget(Paragraph::new(text).block(Block::default().title(" Diff Preview [a apply / n reject] ").borders(Borders::ALL).border_style(focus_style(app.focus==Focus::Diff))).wrap(Wrap{trim:false}),area);
}

fn make_line_diff(old:&[String], new:&[String], max_lines:usize)->String{
    let mut out=String::new();
    let max=old.len().max(new.len());
    let mut shown=0usize;
    for i in 0..max{
        if shown>=max_lines{ out.push_str("\n[diff truncated]"); break; }
        match (old.get(i), new.get(i)){
            (Some(a),Some(b)) if a==b => { out.push_str(&format!(" {:4} {}\n", i+1, a)); shown+=1; }
            (Some(a),Some(b)) => { out.push_str(&format!("-{:4} {}\n+{:4} {}\n", i+1, a, i+1, b)); shown+=2; }
            (Some(a),None) => { out.push_str(&format!("-{:4} {}\n", i+1, a)); shown+=1; }
            (None,Some(b)) => { out.push_str(&format!("+{:4} {}\n", i+1, b)); shown+=1; }
            (None,None)=>{}
        }
    }
    out
...
struct Guard; impl Drop for Guard{fn drop(&mut self){let _=disable_raw_mode(); let _=execute!(io::stdout(),DisableBracketedPaste, LeaveAlternateScreen);}}
fn run()->Result<()> { enable_raw_mode()?; execute!(io::stdout(),EnterAlternateScreen, EnableBracketedPaste)?; let _g=Guard; let mut terminal:Terminal<CrosstermBackend<Stdout>>=Terminal::new(CrosstermBackend::new(io::stdout()))?; let mut app=App::new()?; while !app.should_quit{terminal.draw(|f|ui(f,&mut app))?; if event::poll(Duration::from_millis(100))?{match event::read()?{ Event::Key(k)=>app.on_key(k)?, Event::Paste(p)=>{ for ch in p.chars(){ if app.focus==Focus::Command{ app.command.push(ch); } else if app.focus==Focus::Editor && app.editor_insert{ app.editor_insert_char(ch); } else if app.focus==Focus::Terminal{ app.terminal_input.push(ch); } } }, _=>{} }}} Ok(()) }

fn cli_help_text()->String{
    "terax-tui - terminal-first AI-native dev workspace\n\nUSAGE:\n  terax-tui\n  terax-tui --version\n  terax-tui --doctor\n  terax-tui --help\n\nKEYS:\n  Ctrl+A AI | Ctrl+G Git | Ctrl+T Terminal | Ctrl+E Editor | Ctrl+D Diff | Ctrl+H Help\n\nCOMMANDS:\n  ai <prompt>\n  agent <task>\n  run <cmd>\n  ai-edit-line <instruction>\n  ai-edit-buffer <instruction>\n  ai-write-file <path> <instruction>\n  /project-root /index /memory /init-repo
  clone <url>  open <path>  set-remote <url>  commit <msg>  /agent-commit-message\n\nCONFIG:\n  ~/.config/terax-tui/config.toml\n  Prefer api_key_env over api_key for secrets.\n".into()
}

fn main(){
    let args:Vec<String>=std::env::args().collect();
    if args.iter().any(|a|a=="--version"||a=="-V") { println!("terax-tui {}", env!("CARGO_PKG_VERSION")); return; }
    if args.iter().any(|a|a=="--help"||a=="-h") { print!("{}", cli_help_text()); return; }
    if args.iter().any(|a|a=="--doctor") { print!("{}", doctor_text()); return; }
    if let Err(e)=run(){eprintln!("terax-tui error: {e:#}"); std::process::exit(1)}
}
